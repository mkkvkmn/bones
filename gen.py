...
def add_latest_posts_to_config(
    ...
    if posts:
        ...
        for language, latest_post in latest_posts_by_language.items():
            logger.debug(
                f"Latest post ({language}): {latest_post.get('title', 'Unknown')} "
...
def build_docs(
    ...
    def build_single_doc(doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # (self-referencing templates like feed.xml, sitemap.xml, etc.)
            ...
            return {"success": True, "name": doc.get("name", "unknown")}
        except Exception as e:
            raise RuntimeError(
...
def save_doc(rendered_html: str, doc: Dict[str, Any], config: ConfigType) -> None:
    ...
    if target_file:
        ...
        if not doc_slug:
            raise ValueError(f"Document '{doc.get('title', 'Unknown')}' missing URL")
